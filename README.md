...
To set up the database for the Employee Management System, execute the following SQL queries in your MySQL environment (e.g., phpMyAdmin or MySQL command line):
...
Here's the revised documentation, including the database setup instructions and the SQL query for creating the `employees` table.
...
The Employee Management System is a PHP-based web application designed for managing employee records in a Human Resource Management (HRM) context. It provides functionalities to create, read, update, and delete (CRUD) employee data.

### Features
- **Add Employee**: Ability to add new employee records.
...
- **Database**: MySQL
- **Development Environment**: XAMPP (or similar PHP development environment)
...
- **email**: Employee's email (not nullable, must be unique).
